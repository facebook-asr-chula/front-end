...
class MyRecorder extends Component {
      ...
      if (res.status === 0) {
        if (res.result) {
          ...
          if (res.result.final) {
            ...
            console.log(res.result.hypotheses.map(o => o.transcript));
            __this.callApi(res.result.hypotheses.map(o => o.transcript));
          }
        }
      } else {
  ...
  callApi(result) {
    ...
    axios
      ...
      .then(function(data) {
        if (data.type !== 99) data = JSON.parse(data.replace(/'/g, '"'));
        console.log(data);
        __this.props.onResult(data);
        __this.setState({
          result: data
        });
      })
  ...
  onClick() {
    const { status } = this.state;
    if (status === 'inactive') this.startListening();
    else if (status === 'active') this.stopListening();
  }
  ...
  render() {
    return <RecButton src={this.recorderIamge()} onClick={this.onClick} />;
  }
}
